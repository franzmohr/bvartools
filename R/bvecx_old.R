...
#' 
#' @export
bvecx_old <- function(data, iterations = 50000, burnin = 5000, thin = 10){
  ...
  # Cointegration block
  if(r == "full") {
    Pi_mu_prior <- prior$Pi$constant$mu
    Pi_V_i_prior <- prior$Pi$constant$V_i
    if (tvp[2]) {
      Pi_Q_df_post <- data$priors$Pi$tvp[[1]]
      Pi_Q_V_prior <- data$priors$Pi$tvp[[2]]
    }
    B_mu_prior <- rbind(Pi_mu_prior, B_mu_prior)
    B_V_i_prior <- rbind(cbind(Pi_V_i_prior, matrix(0, n_alpha, n_B)), cbind(matrix(0, n_B, n_alpha), B_V_i_prior))
  } else {
    if (tvp[2]) {
  ...
  if (tvp[1]) {
    y_0 <- y * 0
    #  y_tilde_filter <- y * 0
    B_filter <- matrix(0, n_alpha_B, t + 1)
    B <- B_filter
    B_T <- diag(1, n_B)
    B_Q <- diag(1, n_B)
  ...
  if (r == "full"){
    ...
    #  Pi_filter <- matrix(0, n_alpha, t + 1)
    #  Pi <- matrix(0, n_ect * n, t + 1)
    #  Pi_T <- diag(1, n_alpha)
    #  Pi_Q <- diag(.0001, n_alpha)
    #  Pi_a0 <- matrix(0, n_alpha)
    #} else {
    Pi <- matrix(0, n, n_ect)
    #}
  } else {
  ...
  for (draw in 1:iterations){
    ...
    if (r == "full") {
      if (tvp[1]) {
        ...
        B <- matrix(posterior_normal(y, ect_x, Omega_i, B_mu_prior, B_V_i_prior), n)
        Pi <- B[, 1:n_ect]
        B <- B[, -(1:n_ect)]
        y_dot <- y - Pi %*% ect - B %*% x 
      }
      ...
      if (tvp[1]) {
        if (r > 0) {
          for (i in 1:t) {
            y_0[, i] <- y[, i] - matrix(B_filter[, i], n) %*% ect_x
          }
        } else {
    ...
    #   if (tvp[2]) {
    #     for (i in 1:t) {
    #       y_tilde[, i] <- y[, i] - matrix(Pi[, i], n) %*% ect[, i]
    #     }
    #   } else {
    #     y_tilde <- y - Pi %*% ect 
    #   }
    ...
    #   # Initialise
    #   for (i in 1:t) {
    #     y_tilde_0[, i] <- y_tilde[, i] - Z_B[(i - 1) * n + 1:n,] %*% B_filter[, i]
    #   }
    #   B_0 <- posterior_normal_sur(y_tilde_0, Z_B, Omega_i, B_mu_prior, B_V_i_prior)
    #   y_tilde_filter <- y_tilde - matrix(Z_B %*% B_0, n)
    # 
    #   # Durbin & Koopman (2002)
    #   B_filter <- dk(y_tilde_filter, Z_B, Omega, B_Q, B_T, B_a0, B_Q)
    ...
    #   diag(B_Q) <- 1 / apply(B_res, 1, function(x) {return(stats::rgamma(n = 1, shape = x[2] / 2, rate = x[1] / 2))})
    # } else {
    #   if (shrinkage | sv | (structural & tvp[3])) {
    #     B <- posterior_normal_sur(y_tilde, Z_B, Omega_i, B_mu_prior, B_V_i_prior)
    #     
    #     if (shrinkage) {
    #       B_Gamma <- bvs(y_tilde, Z_x, B, B_Gamma, Omega_i, B_restricted_variables, B_lpr_include_prior, B_lpr_exclude_prior)
    #       B <- B_Gamma %*% B
    #     }
    #   } else {
    #     B <- posterior_normal(y_tilde, x, Omega_i, B_mu_prior, B_V_i_prior)
    #   }
    #   B <- matrix(B, n)
    # }
    # 
    ...
    #     } else {
    #       ect_alpha <- matrix(NA, r, t)
    #       for (i in 1:t) {
    #         ect_alpha[, i] <- crossprod(matrix(beta[, i], n_ect), ect[, i])
    #       }
    #       Z_alpha <- kronecker(t(ect_alpha), diag(1, n))
    #       for (i in 1:t) {
    #         y_alpha_0[,i] <- y_hat[,i] - Z_alpha[(i - 1) * n + 1:n, ] %*% alpha_filter[,i]
    #       }
    #       alpha_0 <- posterior_normal_sur(y_alpha_0, Z_alpha, Omega_i, alpha_mu_prior, alpha_V_i_prior)
    #       y_alpha_filter <- y_hat - matrix(Z_alpha %*% alpha_0, n)
    #       
    #       # Durbin & Koopman (2002)
    #       alpha_filter <- dk(y_alpha_filter, Z_alpha, Omega, alpha_Q, diag(1, n_alpha), matrix(0, n_alpha), alpha_Q)
    #       alpha <- alpha_filter + matrix(alpha_0, n_alpha, t + 1)
    #       
    #       #alpha_res <- cbind(matrix(diag(alpha_Q_V_prior) + rowSums((alpha[,-1] - alpha[, -(t + 1)])^2)), alpha_Q_df_post)
    #       #diag(alpha_Q) <- 1 / apply(alpha_res, 1, function(x) {return(stats::rgamma(n = 1, shape = x[2] / 2, rate = x[1] / 2))})
    #       
    #       #Alpha <- apply(alpha, 2, function(x, n){svd_temp <- svd(matrix(x, n)); return(tcrossprod(svd_temp$u, svd_temp$v))}, n = n)
    #       Alpha <- alpha * 0
    #       for (i in 1:t) {
    ...
    #       
    #       for (i in 1:t) {
    #         Z_beta[(i - 1) *  n + 1:n,] <- kronecker(matrix(Alpha[, i], n), t(ect[, i]))
    #         y_beta_0[, i] <- y_hat[, i] - Z_beta[(i - 1) *  n + 1:n,] %*% beta_filter[,i]
    #       }
    #       beta_0 <- posterior_normal_sur(y_beta_0, Z_beta, Omega_i, beta_mu_prior, beta_V_i_prior)
    #       y_beta_filter <- y_hat - matrix(Z_beta %*% beta_0, n)
    #       
    #       beta_filter <- dk(y_beta_filter, Z_beta, Omega, beta_Q, diag(1, n_beta), matrix(0, n_beta), beta_Q)
    #       beta <- beta_filter + matrix(beta_0, n_beta, t + 1)
    #       
    #       #beta_Q_res <- cbind(matrix(.0001, n_beta) + rowSums((beta[,-1] - beta[,-(t + 1)])^2), t + 3)
    #       #diag(beta_Q) <- 1 / apply(beta_Q_res, 1, function(x) {return(stats::rgamma(n = 1, shape = x[2] / 2, rate = x[1] / 2))})
    #       
    #       for (i in 1:t) {
    #         svd_temp <- svd(matrix(beta[,i], n_ect))
    #         alpha_temp <- matrix(Alpha[,i], n) %*% svd_temp$v %*% tcrossprod(diag(svd_temp$d, r), svd_temp$v)
    #         beta[,i] <- tcrossprod(svd_temp$u, svd_temp$v)
    #         
    #         Pi[, i] <- tcrossprod(alpha_temp, matrix(beta[,i], n_ect))
    #         #Pi[, i] <- tcrossprod(matrix(alpha[, i], n), matrix(beta[, i], n_ect))
    #       } 
    #     }
    #     for (i in 1:t) {
    ...
    #     if (r == "full") {
    #       if ((structural & tvp[3]) | sv) {
    #         Pi <- matrix(posterior_normal_sur(y_hat, kronecker(t(ect), diag(1, n)), Omega_i, Pi_mu_prior, Pi_V_i_prior), n)
    #       } else {
    #         Pi <- matrix(posterior_normal(y_hat, ect, Omega_i, Pi_mu_prior, Pi_V_i_prior), n) 
    #       }
    #     } else {
    #       if (sv) {
    #         Pi_temp <- posterior_cointegration_sur(y_hat, ect, beta, Omega_i, Omega_i, 0, diag(1, n_ect))
    #       } else {
    #         Pi_temp <- posterior_cointegration(y_hat, ect, beta, Omega_i, Omega_i, v_i, P_i)
    #       }
    #       Pi <- Pi_temp[[1]]
    #       beta <- Pi_temp[[3]] 
    #     }
