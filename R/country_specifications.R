...
#' 
#' @param country.data a named list of "ts objects containing the country data.
#' @param global.data a "ts" object with global data.
...
#' @param p.star an integer or vector specifiying the lag order of foreign variables.
#' @param p.global an integer or vector specifiying the lag order of global variables.
...
#' the function \code{\link{country_models}} will automatically reduce each lag order by 1. If a vector is provided,
#' \code{\link{country_models}} will produce a distinct model for all possible specification.
#' 
...
#' If a vector is provided, \code{\link{country_models}} will produce a distinct model for all possible specifications.
#' 
#' @references
...
#'
#' Kastner, G. & Frühwirth-Schnatter, S. (2014). Ancillarity-sufficiency interweaving strategy (ASIS) for boosting MCMC estimation of stochastic volatility models. \emph{Computational Statistics & Data Analysis}, 76, 408–423, doi: 10.1016/j.csda.2013.01.002.
#' 
...
country_specifications <- function(country.data, global.data = NULL, countries = NULL,
                                   p = 2, p.star = 2, p.global = 2, case = "I", type = "VAR",
                                   use = NULL, r = NULL, structural = FALSE, tvp = FALSE, sv = FALSE){
  if (length(tvp) != 3) {
    warning("The argument 'tvp' does not contain three entries. Repeating the first element of current specification three times for the following.")
    tvp <- rep(tvp[1], 3)
  }
  s.vars <- unique(unlist(lapply(country.data, function(x) {return(dimnames(x)[[2]])})))
  if (!is.null(use)){
  ...
  for (i in 1:length(country.data)){
    d.vars <- dimnames(country.data[[i]])[[2]]
    if (!is.null(use)){
    ...
    specs <- c(specs, list(list(domestic.variables = d.vars,
                                ...
                                lags = list(lags = list("domestic" = p, "foreign" = p.star, "global" = p.global),
  ...
  for (i in 1:length(specs)){
    specs[[i]]$domestic.variables <- specs[[i]]$domestic.variables[is.element(specs[[i]]$domestic.variables, dimnames(country.data[[i]])[[2]])]
  }
