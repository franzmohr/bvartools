...
global_series <- function(country.data, country.specs, global.data = NULL){
  ...
  avail.dom.vars <- unique(unlist(lapply(country.data, function(x) {return(dimnames(x)[[2]])}))) # Domestic variables
  if (!is.null(global.data)){
    avail.global.vars <- dimnames(global.data)[[2]] # Global variables
  }
  ...
  for (i in c.names){
    ...
    for (j in country.specs[[i]]$domestic.variables){
      if (is.element(j, dimnames(country.data[[i]])[[2]])){
  ...
  if (length(endo.g.var) > 0){
    if (length(endo.g.var) == length(dimnames(global.data)[[2]])){
      global.data <- NULL
    } else {
      global.data <- global.data[, -which(is.element(endo.g.var, dimnames(global.data)[[2]]))]
    }
  }
