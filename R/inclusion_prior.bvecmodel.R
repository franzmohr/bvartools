...
#' of non-deterministic exogenous variables, between 0 and 1. Default is 0.5.
#' Only used if \code{minnesota_like = TRUE}. See 'Details'.
...
inclusion_prior.bvecmodel <- function(object,
  ...
  if (!is.null(object[["data"]][["train"]][["z"]])) {
    ...
    if (minnesota_like & object[["model"]][["structural"]] & k > 1) {
      n_struct <- k * (k - 1) / 2
      inprior[ncol(z) - n_struct + 1:n_struct] <- kappa2
    }
