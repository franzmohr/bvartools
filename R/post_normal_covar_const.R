...
post_normal_covar_const <- function(y, u_omega_i, k, prior_mean, prior_covariance_i) {
  ...
  temp <- covar_prepare_data(y, u_omega_i, k, tt, FALSE)
  
  # Draw coefficients
  v_post <- prior_covariance_i + crossprod(temp$z, temp$omega_i) %*% temp$z
  mu_post <- solve(v_post, prior_covariance_i %*% prior_mean + crossprod(temp$z, temp$omega_i) %*% temp$y)
