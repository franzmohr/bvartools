...
#' are used.
#' @param x an object of class 'summary.bvarmodel', usually, a result of a call to
#' \code{\link{summary.bvarmodel}}.
#' @param digits the number of significant digits to use when printing.
#' @param ... further arguments passed to or from other methods.
#'
#' @return \code{summary.bvarmodel} returns a list of class 'summary.bvarmodel',
...
summary.bvarmodel <- function(object, ci = .95, period = NULL, ...){
  ...
  if (!is.null(object[["posterior"]][["a"]][["coeffs"]])) {
    if (tvp) {
      ...
      pos <- (period - 1) * nparams + 1:nparams
      temp <- summary(object[["posterior"]][["a"]][["coeffs"]][, pos], quantiles = c(ci_low, .5, ci_high))
    } else {
      temp <- summary(object[["posterior"]][["a"]][["coeffs"]], quantiles = c(ci_low, .5, ci_high)) 
    }
