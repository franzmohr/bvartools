...
write_to_hdf5.bvecmodel <- function(object, filename, ...) {
  ...
  try({
    ...
    ## beta ----
    if (!is.null(object[["priors"]][["beta"]])) {
      if (!"beta" %in% names(group_priors)) {
        group_priors_beta <- group_priors$create_group("beta")
      } else {
        group_priors_beta <- group_priors[["beta"]]
      }
      for (i in names(object[["priors"]][["beta"]])) {
        if (!i %in% names(group_priors_beta)) {
          group_priors_beta[[i]] <- object[["priors"]][["beta"]][[i]]
        }
      }
    }
    ...
    if (!is.null(object[["posterior"]])) {
      ...
      if ("beta" %in% names(object[["posterior"]])) {
        if (!"beta" %in% names(group_posterior)) {
          group_posterior_beta <- group_posterior$create_group("beta")
        } else {
          group_posterior_beta <- group_posterior[["beta"]]
        }
        for (i in names(object[["posterior"]][["beta"]])) {
          if (!i %in% names(group_posterior_beta)) {
            group_posterior_beta[[i]] <- object[["posterior"]][["beta"]][[i]] 
            mcpar_temp <- coda::mcpar(object[["posterior"]][["beta"]][[i]])
            hdf5r::h5attr(group_posterior_beta[[i]], "start") <- mcpar_temp[1]
            hdf5r::h5attr(group_posterior_beta[[i]], "end") <- mcpar_temp[2]
            hdf5r::h5attr(group_posterior_beta[[i]], "thin") <- mcpar_temp[3]
          }
        }  
      }
