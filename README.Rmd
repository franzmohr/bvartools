...
The output of `add_priors` and `add_initial_values` can be used as the input for user-written algorithms for posterior simulation. However, `bvartools` also comes with built-in posterior simulation functions from the [BayesTS](github.com/franzmohr/BayesTS) library. By using function `add_posterior_coefficients`, the model input is forwarded to a posterior function and the output is added to the original object:
...
After the posterior simulation, function `bvar` can be used to collect relevant output of the Gibbs sampler in a standardised object, which can be used by further applications such as `predict` to obtain forecasts or `irf` for impulse respons analysis.
