...
bayests::VecTvpGammaInput read_input(const Rcpp::List &object) {
...
/// The precision only moves with time when there is a covariance block to move
/// it, so which of the two ways it is stored is a question about the model
/// rather than about the entry point.
void read_precision(const Rcpp::List &posterior, const bayests::VecTvpGammaInput &input,
  ...
  if (input.use_psi()) {
    ...
    read_draws_last_period_if_present(block, "coeffs", input.train.periods(input.spec.k), k * k,
                                      draws.u_sigma_inv);
  } else {
...
bayests::VecTvpGammaDraws read_draws_for_forecast(const Rcpp::List &object,
                                                  const bayests::VecTvpGammaInput &input) {
...
Rcpp::List write_draws(const bayests::VecTvpGammaDraws &draws) {
...
// [[Rcpp::export(.VecTvpGammaCoefficients)]]
Rcpp::List VecTvpGammaCoefficients(Rcpp::List object) {

  const bayests::VecTvpGammaInput input = read_input(object);
...
// [[Rcpp::export(.VecTvpGammaForecasts)]]
Rcpp::List VecTvpGammaForecasts(Rcpp::List object) {

  const bayests::VecTvpGammaInput input = read_input(object);
  const bayests::VecTvpGammaDraws draws = read_draws_for_forecast(object, input);
...
// [[Rcpp::export(.VecTvpGammaLogLik)]]
Rcpp::List VecTvpGammaLogLik(Rcpp::List object) {

  const bayests::VecTvpGammaInput input = read_input(object);
  const bayests::VecTvpGammaDraws draws = read_draws_for_loglik(object, input);
  ...
  const arma::mat loglik = bayests::VecTvpGammaSampler().log_likelihood(input, draws);

  return with_posterior_element(object, "loglik", Rcpp::wrap(loglik));
}

// [[Rcpp::export(.VecTvpGammaScore)]]
Rcpp::List VecTvpGammaScore(Rcpp::List object) {
  ...
  const arma::mat score = bayests::VecTvpGammaSampler().predictive_log_density(input, draws);
...
object <- .VecTvpGammaCoefficients(object)
object <- .VecTvpGammaLogLik(object)

object <- add_forecast_input(object, n_ahead = 10)
object <- .VecTvpGammaForecasts(object)
