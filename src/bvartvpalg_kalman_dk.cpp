...
// [[Rcpp::export(.bvartvpalg_kalman_dk)]]
Rcpp::List bvartvpalg_kalman_dk(Rcpp::List object) {
  ...
  arma::mat a, a_AG, a_b, a_init_prior_mu, a_init_prior_vi, a_init_post_v, a_lag, a_mat, a_post_v, a_sigma_v, a_theta0, a_theta1, a_v, zz, zz_bvs;
  arma::sp_mat a_lambda, a_sigma_v_i;
  ...
  if (use_a) {
    ...
    zz = arma::zeros<arma::mat>(tt * k, n_tot); // Final data matrix
    for (int i = 0; i < tt; i++) {
      zz.rows(i * k, (i + 1) * k - 1) = z.rows(i * k, (i + 1) * k - 1);
    } 
    // Measurement
    ...
    a_b = arma::eye<arma::mat>(n_tot, n_tot);
    a_lag = a;
    a_sigma_v = arma::zeros<arma::mat>(n_tot, n_tot);
  ...
  if (sv) {
    ...
    sigma_u = arma::diagmat(exp(h_init));
    sigma_u_i = arma::diagmat(1 / exp(h_init));
  } else {
    omega_i = Rcpp::as<arma::mat>(init_sigma["sigma_i"]);
    sigma_u_i = omega_i;
    sigma_u = arma::solve(Rcpp::as<arma::mat>(init_sigma["sigma_i"]), diag_k);
  }
  ...
  for (int draw = 0; draw < draws; draw++) {
    ...
    if (use_a) {
      ...
      a = bvartools::kalman_dk(y, zz, sigma_u, a_sigma_v, a_b, a_init, a_sigma_v);
      a = a.cols(1, tt);
      ...
      for (int i = 0; i < n_tot; i++) {
        ...
        a_sigma_v(i, i) = 1 / a_sigma_v_i(i, i);
      }
      ...
      for (int i = 0; i < tt; i++) {
        u_vec.subvec(i * k, (i + 1) * k - 1) = y.col(i) - zz.rows(i * k, (i + 1) * k - 1) * a.col(i);
      }
    } else {
      u_vec = yvec;
    }
    ...
    if (sv) {
      ...
      // Draw variances
      for (int i = 0; i < k; i++) {
        h.col(i) = bvartools::stoch_vol(u.row(i).t(), h.col(i), sigma_h(i), h_init(i), 0.0001);
      }
...
model <- gen_var(data = us_macrodata, p = 1,
                 deterministic = "const",
                 tvp = TRUE, sv = FALSE,
                 iterations = 1000, burnin = 1000)

model_with_priors <- add_priors(model,
                                coef = list(v_i = 1 / 10, v_i_det = 1 / 10, shape = 3, rate = .0001),
                                sigma = list(df = "k", scale = .0001))

object <- .bvartvpalg_kalman_dk(model_with_priors)
