...
arma::mat stochvol_ksc1998(arma::mat y, arma::mat h, arma::vec sigma, arma::vec h_init, arma::vec constant) {
  ...
  for (int i = 0; i < k; i++) {
    ...
    log_q = arma::repmat(arma::log(p_i), tt, 1) + arma::log_normpdf(arma::repmat(y.col(i), 1, 7), arma::repmat(h.col(i), 1, 7) + arma::repmat(mu, tt, 1), arma::repmat(sqrt(sigma2), tt, 1));
    log_q.each_col() -= arma::max(log_q, 1);
    q = arma::exp(log_q);
    q = q / arma::repmat(sum(q, 1), 1, 7);
    s = 7 - sum(arma::repmat(arma::randu<arma::vec>(tt), 1, 7) < cumsum(q, 1), 1);
    s = arma::clamp(s, 0, 6); // Guard against a rounding error in the last cumulated weight
