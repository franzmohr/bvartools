...
For what follows, we will only require variables `y` and `x`.
...
```{r}
res_freq <- y - A_freq %*% x
Sigma_freq <- tcrossprod(res_freq) / (t - 9)
round(Sigma_freq, 2)
```
...
library(bvartools)
```
```{r load package 2, echo = FALSE, message=FALSE}
devtools::load_all(".")
```
...
Sigma_V_prior <- diag(.0001, n) # Prior covariance matrix
Sigma_df_post <- t + Sigma_df_prior # Posterior degrees of freedom

# Initial values
Sigma_draw <- diag(.1, n)#rWishart(1, n, Sigma_V_prior)[,,1]
#Sigma_draw <- post_wishart(diag(0, n), Sigma_df_prior, Sigma_V_prior)
Sigma_i_draw <- solve(Sigma_draw)

# Data containers
store <- iter - burnin
draws_A <- matrix(NA, nvars, store)
draws_Sigma <- matrix(NA, n * n, store)
draws_LL <- matrix(NA, t, store)

# Start Gibbs sampler
for (draw in 1:iter) {
  # Draw conditional mean parameters
  A_draw <- posterior_normal(y, x, Sigma_i_draw, A_mu_prior, A_V_i_prior)
  
  # Draw variance-covariance matrix
  res <- y - matrix(A_draw, n) %*% x # Obtain residuals
  Sigma_i_draw <- post_wishart(res, Sigma_df_post, Sigma_V_prior) # Draw inverse Wishart
  ...
  if (draw > burnin) {
    ...
    LL_draw <- gauss_loglik(res, Sigma_draw, Sigma_i_draw)
    draws_LL[, draw - burnin] <- LL_draw
  }
}
```
