...
The considered model is a structural VAR(2) with a constant term:

$$A_0 y_t =  v + \sum_{i = 1}^{2} A_i y_{t-i} + u_t.$$
...
n <- k0 + m # Total number of estimated coefficients
```

Use rather uninformative priors

```{r}
# Set (uninformative) priors
a_mu_prior <- matrix(0, n) # Vector of prior parameter means
a_v_i_prior <- diag(0, n) # Inverse of the prior covariance matrix

sigma_shape_prior <- matrix(0, k) # Prior degrees of freedom
sigma_shape_post <- sigma_shape_prior + tt / 2
sigma_rate_prior <- matrix(0, k)
```

Initial values

```{r}
a0 <- diag(1, k) # Matrix of structural coefficients

ols <- tcrossprod(y, x) %*% solve(tcrossprod(x)) # Reduced form OLS estimate
u <- y - ols %*% x # OLS residuals
ols_vcov <- tcrossprod(u) / tt # OLS variance-covariance matrix

sigma_i <- diag(1, k)
diag(sigma_i) <- 1 / diag(ols_vcov)

z <- kronecker(t(x), diag(1, k))
z <- cbind(z, matrix(0, k * tt, k0))
for (j in 2:k) {
  for (i in 1:tt) {
    z[(i - 1) * k + j, m + (j - 2) * (j - 1) / 2 + 1:(j - 1)] <- -y[1:(j - 1), i]
  }
}
```
...
for (draw in 1:iter) {
  ...
  # Update matrix of structural coefficients
  for (i in 2:k) {
    a0[i, 1:(i - 1)] <- a[m + (i - 2) * (i - 1) / 2 + 1:(i - 1),]
  }
  
  # Errors
  y_hat <- y - matrix(z %*% a, k) # Obtain residuals
  
  # Draw variances
  y_hat_sq <- y_hat^2
  sigma_rate_post <- sigma_rate_prior + rowSums(y_hat_sq) / 2
  for (j in 1:k) {
    sigma_i[j, j] <- rgamma(1, shape = sigma_shape_post[j,], rate = sigma_rate_post[j,])
  }
  
  # Store draws
  if (draw > burnin) {
    draws_a[, draw - burnin] <- a[1:m,]
    draws_a0[, draw - burnin] <- a0
    draws_sigma[, draw - burnin] <- solve(sigma_i)
  }
}
```
