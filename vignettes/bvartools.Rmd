...
* *Setting of prior hyperparameters*: Generates prior matrices for a given model.
* *Setting initial values*: Generates initial values for a given model.
* *Posterior simulation*: Researchers can choose to use the posterior simulation algorithms of the package or use their own algorithms.
...
```{r}
model_with_priors <- add_initial_values(model_with_priors, method = "ols")
```
...
a_mu_prior <- model_with_priors$priors$a$mu # Prior means
a_v_i_prior <- model_with_priors$priors$a$v_i # Prior precisions
