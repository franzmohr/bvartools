...
```{r}
rescale_factor %*% matrix(object[["initial"]][["a"]][1:2])
round(matrix(object[["initial"]][["a"]], 2), 3)
```
...
rescale_factor <- diag(attr(w, "sd"))
rescale_factor_inv <- solve(rescale_factor)
...
for (draw in 1:draws) {
  ...
  # Store draws
  if (draw > burnin) {
    draws_alpha[, draw - burnin] <- rescale_factor %*% alpha
    draws_beta[, draw - burnin] <- rescale_factor_inv %*% beta
...
```{r, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=10}
plot(bvec_est)
```
...
# Genrate forecasts
bvar_pred <- predict(bvar_form, n.ahead = 10)
