...
The illustration uses `us_macrodata`, quarterly US CPI inflation (`Dp`), the unemployment rate (`u`) and the Fed Funds rate (`r`) from 1959Q2 to 2007Q4, and asks the question these series are usually asked: what does a contractionary monetary policy shock do?
...
``` r
model <- create_bvarmodel(us_macrodata, p = 4, deterministic = "const",
...
A contractionary monetary policy shock is conventionally taken to raise the policy rate, lower inflation and raise unemployment. Here that pattern is imposed for the impact quarter and the two that follow.
...
The first three quarters of each panel are the restrictions themselves and carry no information: the responses have the sign they have because that is what was imposed. What the restrictions do not determine is everything from the fourth quarter on, and there the inflation response stays negative well beyond the restricted window, while the rate returns to zero within a year.
...
The recursive responses show the *price puzzle* of Sims (1992): inflation rises for about two quarters after a contractionary shock, and the credible interval excludes zero, which is the opposite of what a policy tightening is supposed to do. They also begin at exactly zero, because ordering the policy rate last assumes that inflation cannot react within the quarter.
...
Chan, J., Koop, G., Poirier, D. J., & Tobias, J. L. (2020). *Bayesian econometric methods* (2nd ed.). Cambridge: Cambridge University Press.
